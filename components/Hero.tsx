...
function HeroSVG() {
  return (
    ...
    >
      ...
      <circle cx="440" cy="440" r="2" fill="white" fillOpacity={0.1} />
    </svg>
  );
}
...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center bg-[#0a0a0a] noise pt-16 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 w-full py-24 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div className="flex flex-col gap-6">
          ...
          <h1 className="hero-headline text-white" aria-label="We build AI that actually ships.">
            {words.map((word, i) => (
              <motion.span
                key={i}
                custom={i}
          ...
          >
            <a
              href="#contact"
              className="inline-flex items-center justify-center h-12 px-7 rounded-lg bg-white text-[#0a0a0a] text-sm font-bold hover:bg-white/90 transition-colors duration-200"
            >
              Book a Free Call
            </a>
            <a
              href="#services"
              className="inline-flex items-center justify-center h-12 px-7 rounded-lg border border-white/20 text-white text-sm font-semibold hover:bg-white/5 transition-colors duration-200"
            >
              See Our Services
            </a>
          </motion.div>
